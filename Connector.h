...
typedef struct _ConnectMsg
{
  
  int FromCell;  // Index into the source node's global cell list
  int FromCmp;   // Index into the source cell's compartment list
  int Delay;     // Number of timesteps that must elapse before the synapse firing can arrive
  void *Addr;    // Memory address of the synapse on the destination node
} CONNECTMSG;
 ...
 */
void outputConnectMap( CONNECT *CD, char *Map, int reversed=0 );
...
void MakeRecurrentConnects( CONNECT *CD, char *Map, int nMade, CONNECTMSG *CMBuf,
                            ...
                            double *dest, int nDestCells, double *distanceMap,
