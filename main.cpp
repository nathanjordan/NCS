...
/* for construction.                                                         */
  ...
  if (NI->AR->Brain->loadfile != NULL) //restored a brain need to patch pointers
  {
    AR = NI->AR;
    NI->job = AR->Brain->job;
    if (NI->job == NULL) NI->job = "job";
...
/* Now the clusters can be distributed to the nodes                          */
  ...
  if (AR->Brain->loadfile == NULL)
  {
    printf ("Barrier before Distributor\n");
...
/* Now build the brain: that is, create the cells &c that are on this node   */
  ...
  if (AR->Brain->loadfile == NULL)
    ...
    TheBrain->Load (AR->Brain->loadfile, nothing);
...
//  Connector (AR, RM);
                                   /* A pointer is not a logical value! */
  if (AR->Brain->loadfile == NULL) //this doesn't need to be done if loading a brain
    Connector (AR, RM);
...
/* Now that the brain is complete, create stimuli & reports from input */
  ...
  TheBrain->checkForSynapseOverrides( AR );
