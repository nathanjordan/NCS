...
/* Stuff that was in Syn_Learn  */
  ...
  if (AR->SynLearn != NULL)
  {
    ...
    if (iSLN != NULL)
    {
      ...
      baseLearnShape = iSLN->Learning_Shape;
      NegDeltaUSE  = iSLN->Neg_Heb_Peak_Delta_Use[0];
      PosDeltaUSE  = iSLN->Pos_Heb_Peak_Delta_Use[0];
    }
  }
...
void SynapseDef::ToggleLearning( int pMode )
{
    //if learning is none, revert to base learning (may still be none)
    if( !pMode )
        LearnNeg = LearnPos = 0;
    else
    {
        if( baseLearnCode == 0 )
            LearnNeg = LearnPos = 0;
        else if( baseLearnCode == 1 )
            LearnPos = !(LearnNeg = 0);
        else if( baseLearnCode == 2 )
            LearnPos = !(LearnNeg = 1);
        else if( baseLearnCode == 3 )
            LearnPos = LearnNeg = 1;
    }
    
    cerr<<"toggled learning to: +Hebbian="<<LearnPos<<" -Hebbian="<<LearnNeg<<endl;
}
/*-------------------------------------------------*/

void SynapseDef::ToggleSFD( int pMode )
{
    //if learning is none, revert to base learning (may still be none)
    if( !pMode )
        RSEType = 0;
    ...
    cerr<<"toggled SFD to: "<<RSEType<<endl;
}
/*-------------------------------------------------*/
