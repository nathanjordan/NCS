...
//  CONNECTMSG *XmitPtr;
  ...
  int SynIdx, nXmit, nRecv, nMade, *CelIDs, *CmpIDs;
  int blen [5], nFrom, nTo;
//  int CelID, CmpID, nSent, del;
...
/* are idle.                                                                 */
  ...
  printf ("Connect %4d: Seed = %8d\n", __LINE__, CellMgr->CDList [0].Seed);
  for (n = 0; n < nCD; n++)
  {
    if (Order [n] == n)
    {
...
/* tCells is temporary collecting area for connect stuff */
  ...
  printf ("Connect, created %4d tCells\n", TheBrain->nCells);
...
//  MPI_Type_commit (&MsgType);
  ...
  type [1] = MPI_LONG;
  blen [1] = 1;
  MPI_Address (&(dummyCM.FromCell), &(disp [0]));
  MPI_Address (&(dummyCM.Addr),     &(disp [1]));
  disp [1] -= disp [0];
  disp [0] -= disp [0];

  MPI_Type_struct (2, blen, disp, type, &MsgType);
  MPI_Type_commit (&MsgType);
...
// The code statement below avoids having same connection setup for different
// cell groups on more than one node. All cell groups now have unique
// connection map, irrespective of number of nodes used for simulation.
// Fixed by James Frye and Dr. Goodman. (Jan. 1, 2007)
    RN->SeedIt (CD->Seed + i);
    ...
    if (CTo->Node == NI->Node)
    {
      ...
      if( CD->iCon->recurrentProbability[0] > 0 || CD->iCon->recurrentProbability[1] > 0 )
        MakeRecurrentConnects( CD, Map, nMade, CMBuf, CelIDs, CmpIDs, allPositions[CFrom->Node],
                               ...
                               distanceMap, ConComm, status, MsgType );

      //output map
      if( NI->AR->Brain->flag & OUTPUT_CONNECT_MAP_FLAG )
        outputConnectMap( CD, Map );
...
//    PrintMap (CFrom->Qty, CTo->Qty, Map);
      createSynapses( CD, Map, distanceMap, nMade, ConComm, status, MsgType );
/*
...
// @@@ Modified by Milind Z, for double to int conversion issue.
//        del = (int) (NI->FSV * RN->RandRange (iSyn->SynDelay));
	del = (int) round(((double)NI->FSV) * RN->RandRange (iSyn->SynDelay));
...
/* info to arrive.  CD->nCon contains the number of items that are expected  */
    ...
    else if (CFrom->Node == NI->Node)
    {
        ...
        if( CD->iCon->recurrentProbability[0] > 0 || CD->iCon->recurrentProbability[1] > 0 )
            acceptRecurrentConnection( CD, Map, distanceMap, ConComm, status, MsgType );
  ...
  for (i = 0; i < TheBrain->nCells; i++)
  {
    ...
    for (j = 0; j < Cel->nCompartments; j++)
    {
      ...
      if (Cmp->nSynapse > 0)
      {
        ...
        if (tCmp->Syn [n]->SynDef->LearnPos) Cmp->iOtherSynapse++;
...
void MakeRecurrentConnects( CONNECT *CD, char *MapAtoB, int nMade, CONNECTMSG *CMBuf,
                            ...
                            double *dest, int nDestCells, double *distanceMapAtoB,
                            MPI_Comm &ConComm, MPI_Status &status, MPI_Datatype &MsgType )
{  
    ...
    if( CD->To->Node == CD->From->Node )  //No need to communicate via MPI
        createSynapses( CD, MapBtoA, distanceMapBtoA, nMadeBtoA, ConComm, status, MsgType );
    else
    {
...
// @@@ Commented by Milind Z. Jan. 25, 2007. Because it was a Debug printf by
// James Frye.
//  printf ("MakeConnects\n");
...
// @@@ Modified by Milind Z. Dec. 22, 2006.
// Moved the Random Number Generator seeding call to outermost for loop.
// Done to avoid getting the same pattern of connections between
// different layers or cell groups.
//  RN->SeedIt (CD->Seed);
...
// @@@ Commented by Milind Z. Jan. 25, 2007. Because it was a Debug printf by
// James Frye.
//        if (n < 10) printf ("  %4d - %4d\n", CMBuf [n].FromCell, CelIDs [n]);
        n++;
      }
    }
  }
...
void outputConnectMap( CONNECT *CD, char *Map, int reversed )
{
    ...
    if( NI->Node == 0 ) //do I have the map, or do I need to get it?
    {
        ...
        // New connection map format code
        FILE *fConnectMap=NULL;
        // Do sufficient memory allocation
        char fname[1024];

        sprintf( fname, "%s#%s.%s.%s.%s#to#%s.%s.%s.%s#%s.connectmap.txt", NI->job, NI->AR->Columns[ A->iCol ]->L.name,
				NI->AR->Layers [ A->iLay ]->L.name, NI->AR->Cells [ A->iCel ]->L.name, cmpA, 
				NI->AR->Columns[ B->iCol ]->L.name, NI->AR->Layers [ B->iLay ]->L.name,
				NI->AR->Cells [ B->iCel ]->L.name, cmpB, CD->iCon->SynName);

        fConnectMap = fopen( fname, "w" );
        if( !fConnectMap )
            fprintf( stderr, "Error: could not open %s for output of connect maps\n", fname );

        // Writing to report file only those source and destination cells that have a connection.
        // ie. Indices in connection matrix with value 1 (A connection) are recorded. 0 valued (No connection) are ignored.
//        fprintf( fConnectMap, "Source Destination\n");
        int rowOffset=0;
        // Traversing data by Source*Destination. But data in Map[] is stored as a 1-D array in Source*Destination format (Row Major Order).
        // So, accessing data column-wise instead of row-wise (similar to Column Major Order?).
        for( int colIndex=0; colIndex<qtyA; colIndex++ )
        {
            rowOffset=0;
            for( int rowIndex=0; rowIndex<qtyB; rowIndex++ )
            {
                // Assumption: Map[] has number 0 or 1 which translate to ascii 0 or 1, not characters.
                // (Map is a char array). To overcome this, (int)'0' is added to value of Map[] to make it a character value.
                if (Map[colIndex+rowOffset] == 1)
                    // +1, since numbering starts from 0
                    fprintf( fConnectMap, "%d %d\n", (colIndex+1), (rowIndex+1) );
                rowOffset+=qtyA;
            }
        }
...
void acceptRecurrentConnection( CONNECT *CD, char *map, double *distanceMap,
                                MPI_Comm &ConComm, MPI_Status &status, MPI_Datatype &MsgType )
{
    int count = CD->To->Qty * CD->From->Qty;
    int bytesReceived;
...
void createSynapses( CONNECT *CD, char *map, double *distanceMap, int nMade,
                     MPI_Comm &ConComm, MPI_Status &status, MPI_Datatype &MsgType )
{
    SynapseDef* iSyn = TheBrain->SynapseDefs [CD->iCon->SynType];
    const int nXmit = XMIT_SIZE / sizeof (CONNECTMSG);
        
    CONNECTMSG *CMBuf = new CONNECTMSG[nMade];
    
    //allocating and deleting this memory seems costly.  Should I allocate this once outside the
    // function and pass in as another argument (the function call stack can get high)
    int *CelIDs = new int[nMade];
    int *CmpIDs = new int[nMade];
    
    // Use the map to consolidate any connected cells into an array
    gatherConnectedIDs( CD, map, CMBuf, CelIDs, CmpIDs );
	...
	int CmpID = CmpIDs [j];
        ...
        Compartment *Cmp  = TheBrain->Cells [CelID]->Compartments [CmpID];
        TMPCMP *tCmp = &(tCells [CelID].Cmp [CmpID]);

        Synapse *sp = new Synapse (iSyn, Cmp, RN);
        //If the USE value is supposed to be read from a file, parse the next one and give it to this synapse
        //sp->setUSE( parseUSE( buffer ) );
...
//        delay = (int) (NI->FSV * RN->RandRange (iSyn->SynDelay));
        int delay = (int) round(((double)NI->FSV) * RN->RandRange (iSyn->SynDelay));
    ...
    //destination node sends synapse info to source node so it can have the memory addresses
    // ( if destination node is same as source node, no MPI is needed )
