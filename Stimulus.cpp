...
Stimulus::Stimulus (T_STIMULUS *iStim, T_STINJECT *iStInj, Random *RNG)
{
  ...
  RN = RNG;
  Seed = iStim->Seed;	// not actually function here, but Random maybe 
...
//printf ("Stimulus::Stimulus: pattern = %s, id = %d\n", NameDefine (PATTERN), id);
  ...
  switch (PATTERN)
  {
    ...
    case FILEBASED: GetInputCells (iStInj);
        ...
        break;
    case FILEBASEDDIRECT: GetInputCells (iStInj);
                    if (nThings == 0)
                      id = -1;        /* Flag so caller will delete */
                   else
                    {
                        Func = &Stimulus::FileDirectStim;
                    }
         break;
    ...
    case NOISE:     if (GetCells (iStInj) > 0)
		    {
                      ...
                      Rate = (double)(iStim->Rate / NI->FSV);		// actual firing rate
                      Tau = (double) (NI->FSV * iStim->Tau);		// decay constant in ticks
                      Correl = (iStim->Correl <= 1) ? iStim->Correl : 1;

		      if (Tau > 0)					// Inhomogeneous
		      {
                        // allocate channels and assign firing rates 
                        Channels = (double *) calloc (nThings, sizeof (double));
                        for(i = 0; i < nThings; i++)
                          Channels[i] = Rate;

                        if (Correl > 0)
                          Func = &Stimulus::InhomoCorr;			// Correlated
                        else
                          Func = &Stimulus::InhomoUnCorr;		// Uncorrelated
                      } else {					// Homogeneous
                        if (Correl > 0)
                        {						// Correlated
                          // cllocate independent channels
                          NumChan = (int) (nThings + sqrt(Correl) * (1 - nThings));
                          Channels = (double *) calloc (NumChan, sizeof (double));
                          for(i = 0; i < NumChan; i++)
                            Channels[i] = MinPreValue;

                          Func = &Stimulus::HomoCorr;
                        }
                        else						// Uncorrelated
                          Func = &Stimulus::HomoUnCorr;
		      }
		    }
	    	    break;
...
//printf ("Stimulus::~Stimulus: deleting self\n");
  ...
  if ((Data != NULL) && (PATTERN != FILEBASED) && (PATTERN != FILEBASEDDIRECT)) free (Data);
...
void Stimulus::ResetStim ()
{
  ...
  TimeChange = -1000000;			// as negative as possible
  Noise = 0;
  MaxPreValue = 30;
  MinPreValue = -80;
}
...
/* *-----------------------------------------------------------------*/
/*   Function:     FileCurrentStim                                        
 *   Programmer:   Oscar A. Sessions                                      
 *   Date Updated:10/5/06                                                                          
		Actions: This function calculates the value to 
              	send to the message bus for the cell 
   	       	to be stimulated.  The function 
	       	uses Data wich is a two dimensional array 
               	read in from an input file.  The 	  	 
               	function that reads the file is 
	       	DoStim::ReadFileCurrentStim().                                                   
*/

void Stimulus::FileDirectStim (int TimeStep)
{

  int idata ;
  int i;
  int CurrentColumn = 0; //used to hold the index value for the column
  double Value;

  idata = (TimeStep-TStart [iTime]) % nDataRows;
  //idata = index of row in data file
  //nThings = number of cells to recieve data 
  
  
  for (i = 0; i < nThings; i++)
  {  
    CurrentColumn = i % nFreqs; //this calculates wich cell will get wich frequency    
    Value = *(Data + idata * nFreqs + CurrentColumn ); //lookup Value row column
    
    //printf("row = %i column = %i cell = %i timestep = %i ",idata,current_freq,i,TimeStep);
    //printf(" Value = %f \n", Value);
    //
    //I noticed that this could send up to nThings messages on the message bus
    //while the function FileStim() is set up to only send one message with an array
    //of cells to recieve stimulus data.
...
void Stimulus::InhomoCorr (int TimeStep)	// inhomogenous correlated Poisson
{
  int i;

  if (TimeStep >= TimeChange)			// outside one interval of time duration
  {
    TimeChange = TimeStep + Tau * RN->Expo();	// genrate next interval
    Noise = RN->Gauss();			// generate noise term

    for (i = 0; i < nThings; i++)
    {
      Channels[i] = Rate * (1 + (RN->Gauss() + Noise) * Correl);	// update firing rate
      if (Channels[i] < 0) Channels[i] = 0;

      if (RN->Rand() < Channels[i])
        MsgBus->SendStimMsg(TimeStep, 1, &(ThingPtrs[i]), MsgType, MaxPreValue);
      else
        MsgBus->SendStimMsg(TimeStep, 1, &(ThingPtrs[i]), MsgType, MinPreValue);
    }
  } else {					// inside one interval of time duration
    for (i = 0; i < nThings; i++)
    {
      if (RN->Rand() < Channels[i])
        MsgBus->SendStimMsg(TimeStep, 1, &(ThingPtrs[i]), MsgType, MaxPreValue);
...
void Stimulus::InhomoUnCorr (int TimeStep)	// inhomogenous uncorrelated Poisson
{
  int i;
  ...
  if (TimeStep >= TimeChange)			// outside one interval of time duration
  {
    ...
    for (i = 0; i < nThings; i++)
    {
      Channels[i] = Rate * (1 + RN->Gauss() );	// update firing rate
      if (Channels[i] < 0) Channels[i] = 0;
...
void Stimulus::HomoUnCorr (int TimeStep)	// homogenous uncorrelated Poisson
{						// easy if no correlated
  int i;
...
void Stimulus::HomoCorr (int TimeStep)		// homogenous correlated Poisson
{						// no easy if correlated
  int i;

  for (i = 0; i < NumChan; i++)	// reset all independent channels
      Channels[i] = MinPreValue;

  for (i = 0; i < NumChan; i++)	// distribute among independent channels
    if (RN->Rand() < Rate)
      Channels[i] = MaxPreValue;

  // distribute independent channels among output channels
  for (i = 0; i < nThings; i++)
      MsgBus->SendStimMsg(TimeStep, 1, &(ThingPtrs[i]), MsgType, Channels[(int) (RN->iRand()%NumChan)]);
}
