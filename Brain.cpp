...
int Brain::DoThink ()
{
  ...
  printf ("Preparing to check for sfd toggles\n");
  fflush (stdout);

  // Added 8/12/10 Joshua Hegie 
  //before entering the main loop, see if any synapse types will be toggling
  // SFD.  Put those into a priority_queue so that they can be quickly 
  // checked and enacted
  priority_queue<SynapseToggle> sfdToggles = checkForSfdToggles();

  printf ("There are %i SFD toggles\n", sfdToggles.size());
  
  //before entering the main loop, see if any synapse types will be toggling
  // hebbian.  Put those into a priority_queue so that they can be quickly 
  // checked and enacted
  priority_queue<SynapseToggle> toggles = checkForSynapseToggles();
  ...
  {     
    ...
    //apply any SynapseToggles
    if( !toggles.empty() )
    {
        while( (int) (toggles.top().getTime()*NI->FSV+EPSILON) <= TimeStep && !toggles.empty() )
        {
            //cerr<<"Applying synapse toggle at time "<<TimeStep<<endl;
            toggles.top().getSynapseDef()->ToggleLearning( toggles.top().getMode() );
            toggles.pop();
        }
    }
    ...
    if( !sfdToggles.empty() )
    {
        while( (int) (sfdToggles.top().getTime()*NI->FSV+EPSILON) <= TimeStep && !sfdToggles.empty() )
        {
            //cerr<<"Applying synapse toggle at time "<<TimeStep<<endl;
            sfdToggles.top().getSynapseDef()->ToggleSFD( sfdToggles.top().getMode() );
            sfdToggles.pop();
        }
    }
    
    //aplly any synapse overrides - for better efficiency, synapse toggles and use overrides should probably be
    // merged into one object, or rather a parent class of SynapseEvent type created which the toggles and overrides can be subclassed from
    if( !overrides.empty() )
    {
        while( (int) (overrides.top().getTime()*NI->FSV+EPSILON) <= TimeStep && !overrides.empty() )
        {
            overrides.top().execute();
            overrides.pop();
        }
    }
...
priority_queue<SynapseToggle> Brain::checkForSynapseToggles()
{
    vector<SynapseToggle> collection;

    //for each synapse, check for 
    for( int synapseIndex=0; synapseIndex<NI->AR->nSynapse; synapseIndex++ )
    {
        if( SynapseDefs[synapseIndex]->source->nToggles > 0 )
        {
            for( int toggleIndex=0; toggleIndex<SynapseDefs[synapseIndex]->source->nToggles; toggleIndex++ )
                collection.push_back( SynapseToggle( SynapseDefs[synapseIndex],
                    SynapseDefs[synapseIndex]->source->toggleTimes[toggleIndex],
                    SynapseDefs[synapseIndex]->source->toggleModes[toggleIndex] ) );
        }
    }
    
    //The command that will get executed
    //SynapseDefs[i]->SetLearning( LearnCode( result[n+1] ) );
    
    return priority_queue<SynapseToggle>( collection.begin(), collection.end() );
}

//---------------------------------------------------------------------------------------------------------

// Added 8/12/10 Joshua Hegie
priority_queue<SynapseToggle> Brain::checkForSfdToggles()
{
    vector<SynapseToggle> collection;
    ...
    for( int synapseIndex=0; synapseIndex<NI->AR->nSynapse; synapseIndex++ )
    {
        if( SynapseDefs[synapseIndex]->source->nSfdToggles > 0 )
        {
            for( int toggleIndex=0; toggleIndex<SynapseDefs[synapseIndex]->source->nSfdToggles; toggleIndex++ )
                collection.push_back( SynapseToggle( SynapseDefs[synapseIndex],
                    SynapseDefs[synapseIndex]->source->sfdToggleTimes[toggleIndex],
                    SynapseDefs[synapseIndex]->source->sfdToggleModes[toggleIndex] ) );
        }
    }
...
void Brain::checkForSynapseOverrides( ARRAYS *AR )
{
    vector<SynapseOverride> collection;
    int realEventIndex = 0;
    
    //Only use events that were named in the Brain section -> will reindex into actual event list
    for( int eventIndex=0; eventIndex<AR->Brain->nEvents; eventIndex++ )
    {
        realEventIndex = AR->Brain->Events[eventIndex];
        //This is inefficient since I will create two vectors in the SynapseOverride's constructor, then
        // copy them into the the collection's vector, then again into the queue!
        SynapseOverride potential( AR->Events[ realEventIndex ], CellMgr, this );
        if( !potential.empty() )
            collection.push_back( potential );
    }
        
    overrides = priority_queue<SynapseOverride>( collection.begin(), collection.end() );
}
...
int Brain::CommandCheck()
{
  ...
  if( NI->Node == 0 )
  {
	// Setup select statement to check if data is available.
	// We could do this with a non-blocking read as well.
	FD_ZERO(&cmdfds);
	FD_SET(sd, &cmdfds);
	select(sd+1,&cmdfds,NULL,NULL,&tv);	// Return Immediately

	while (FD_ISSET(sd, &cmdfds)) {
