...
class Brain
{
    private:
        /**
         * If any synapse types will need the USE values overridden at some point in the simulation,
         * a synapseOverride event will be created and inserted into this priority queue so that only
         * the earliest one is checked, rather than tracersing a list
         */
        priority_queue<SynapseOverride> overrides;
...
/* synapse definitions */
  ...
  FILE *setupPort();
    ...
    /**
     * During initialization, any SynapseToggle events that were specified in the input file
     * should be gathered and sorted into a priority queue.
     *
     * @return the priority queue that contains the toggle events
     */
    priority_queue<SynapseToggle> checkForSynapseToggles();
    
    // Added 8/12/10 Joshua Hegie
    priority_queue<SynapseToggle> checkForSfdToggles();
    
    /**
     * During initialization, any SynapseOverride events that were specified in the input file
     * should be gathered and sorted into a priority queue.
     *
     * @param AR reference to data collected during parsing, where any SynapseOverride events are stored
     */
    void checkForSynapseOverrides( ARRAYS *AR );
