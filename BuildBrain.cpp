...
/* on this node.                                                             */
  ...
  printf ("BuildBrain: nLocalCells = %d\n", TheBrain->nCells);
...
/* at the peak value.  (Wouldn't tables like the PSGs work better?)          */
  ...
  for (i = 0; i < AR->nSynLearn; i++)
  {
    ...
    // choose which modification curve to use
    if (iSL->Learning_Shape == 0)
    {
      ...
      v = (nPre ==0) ? iSL->Neg_Heb_Peak_Delta_Use [0] : 0.0;

      //v = 0.0; if nPre == 0, increment should start from -peak value
      for (j = 0; j < nPre; j++)
      {
    ...
    if (iSL->Learning_Shape == 1)
    {
      tau = (double) NI->FSV * iSL->Neg_Heb_Peak_Time[0];

      for (j = 0; j < iSL->nNegLearn; j++)
        iSL->NegLearnTable [j] = exp( (double) -j / tau );
    }
...
/* Repeat process for positive learning */
    ...
    {  
      ...
      v = (nPre ==0) ? iSL->Pos_Heb_Peak_Delta_Use [0] : 0.0;

      //v = 0.0; if nPre == 0, decrement should start from +peak value
      for (j = 0; j < nPre; j++)
      {
    ...
    if (iSL->Learning_Shape == 1)
    {
      tau = (double) NI->FSV * iSL->Pos_Heb_Peak_Time[0];

      for (j = 0; j < iSL->nPosLearn; j++)
        iSL->PosLearnTable [j] = exp( (double) -j / tau );
    }
...
void outputCellPositions( int offset, CLUSTER *cluster, FILE *fout )
{
    ...
    printf ("BuildBrain:outputCellPositions called\n");
