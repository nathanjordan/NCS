...
//printf ("Synapse::Synapse:\n);
  ...
  //These are 1 for Triangle Shape. Nearest Neighbour rule
  if (SynDef->baseLearnShape == 0) { limitPosLearning = limitNegLearning = 1; }
  //These are bigger values for Exponential Shape. 
  if (SynDef->baseLearnShape == 1) { ModPosLearning = ModNegLearning = 0; }
...
void Synapse::DoNegativeLearning ( int TimeStep )
{
  int dt;

  //check to see if the negative learning flag is on (may be turned off during run)
  if (SynDef->LearnNeg != 1)
    return;

  if (SynDef->baseLearnShape)						// Exponential
  {
    // Update Postive Modulator for Exponential learning curve
    dt = TimeStep - PreviousSpikeTime;
    if (dt < SynDef->nPosLearn)
      ModPosLearning = ModPosLearning * SynDef->PosLearnTable [dt] + SynDef->PosDeltaUSE;
    else 
      ModPosLearning = SynDef->PosDeltaUSE;

    // Calculate Negative Learning
    dt = TimeStep - Cmp->LastFireTime;
    if (dt < SynDef->nNegLearn)
    {
      USE -= (SynDef->NegLearnTable [dt] * ModNegLearning);	// additive rule
      if (USE < 0.0) USE = 0.0;
      counterNegLearning++;
    }
  } else {								// Triangle 
    if( counterNegLearning < limitNegLearning )
    {
      // Calculate Negative Learning
      dt = TimeStep - Cmp->LastFireTime;
      if (dt < SynDef->nNegLearn)
      {
        USE -= (SynDef->NegLearnTable [dt]);
        if (USE < 0.0) USE = 0.0;
        counterNegLearning++;
      }
    }
...
void Synapse::DoPositiveLearning (int TimeStep)
{
  ...
  if (SynDef->baseLearnShape)					// Exponential
  {
    // Update the Negative modulator for Exponential Learning curve;
    dt = TimeStep - Cmp->LastFireTime;
    if (dt < SynDef->nNegLearn)
      ModNegLearning = ModNegLearning * SynDef->NegLearnTable[dt] + SynDef->NegDeltaUSE;
    else
      ModNegLearning = SynDef->NegDeltaUSE;
  
    // Calculate Postive Learning
    dt = TimeStep - PreviousSpikeTime;
    if (dt < SynDef->nPosLearn)
    {
      USE += (SynDef->PosLearnTable [dt] * ModPosLearning);	// additive rule
      if (USE > 1.0) USE = 1.0;
      counterPosLearning++;
    }
  } else {							// Triangle
    if ( counterPosLearning < limitPosLearning )
    {
      // Calculate Postive Learning
      dt = TimeStep - PreviousSpikeTime;
      if (dt < SynDef->nPosLearn)
      {
        USE += (SynDef->PosLearnTable [dt] );			// additive rule
        if (USE > 1.0) USE = 1.0;
         counterPosLearning++;
      }
    }
