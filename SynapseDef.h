...
class SynapseDef
{
 public:
  ...
  int baseLearnCode;
          ///Flag to indicate which modification curve to use for the learning
  int baseLearnShape;
  ...
  double NegDeltaUSE;
          ...
          /// SFD storage, so that the mode can be changed without loss of the original
  int sfdMode;
  ...
  /**
   *  Switch SynapseDef's learning method between what it was declared with or NONE
   *
   * @param pMode 1 if Synapse should use it declared learning mode, 0 it should use NONE
   */
  void ToggleLearning( int pMode );

  // Added 8/12/10 Joshua Hegie
  // Toggles SFD learning on or off
  void ToggleSFD ( int pMode );
