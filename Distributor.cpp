...
void Distributor (char *alg)
{
  ...
  else if (strcasecmp (alg, "ByColumn") == 0)
  {
    if (NI->Node == 0)
      printf ("Distributor::Distributor: Using '%s ' algorithm\n", alg);
    ByColumn ();
...
/*---------------------------------------------------------------------------*/
/* This tries to assign an equal number of synapses (and thus memory) to     */
/* each node, regardless of cpu power. For large models, so all memory is    */
/* used.                                                                     */
...
/* This tries to assign an equal number of synapses (and thus memory) to     */
/* each node, regardless of cpu power. For large models, so all memory is    */
/* used. Sort clusters by size, and assign in that order.                    */
...
/* Here the weight is memory usage of both synapse and sendto.               */
  ...
  if (NI->Node == 0) printf ("EqualOrder Clusters (%d)\n", CellMgr->nClusters);
  weight = 0.0;
  for (i = 0; i < CellMgr->nClusters; i++)
  {
    CS = CellMgr->Clusters [i];
    CS->weight = (double) (CS->nSyn * sizeof (Synapse))
               + (double) (CS->nSendTo * sizeof (SENDITEM));
    weight += CS->weight;
    if (NI->Node == 0) printf ("%4d: %4d, %4d, %4d\n", i, CS->iCol, CS->iLay, CS->iCel);
  }
...
//printf ("-- Assigning CellIDs, nClusters = %d\n", CellMgr->nClusters);
  for (i = 0; i < CellMgr->nClusters; i++)
  {
    CS = CellMgr->Clusters [i];
...
//    printf ("%4d: %3d  (%5d, %5d)\n", i, iNode, CS->idx, CS->Qty);
  ...
  if (NI->Node == 0)
  {
    printf (" Cluster  Col  Lay  Cell  CPU\n");
    for (i = 0; i < CellMgr->nClusters; i++)
    {
      CS = CellMgr->Clusters [i];
      printf ("%4d: %4d, %4d, %4d\n", i, CS->iCol, CS->iLay, CS->iCel, CS->Node);
    }
  }
...
/*---------------------------------------------------------------------------*/
/* This assigns all the clusters of a column to a CPU. If there are more     */
/* columns than CPUs, additional ones are assigned round-robin, with no      */
/* attempt to load-balance.                                                  */

static void ByColumn ()
{
  CLUSTER *CS;
  double weight, *CPUweight;
  int i, iNode, nCols, *LocalIDs;

/* First compute the weight of each cluster, and the number of columns   */
  ...
  for (i = 0; i < CellMgr->nClusters; i++)
  {
    ...
    CS->weight = (double) (CS->nSyn * sizeof (Synapse))
               + (double) (CS->nSendTo * sizeof (SENDITEM));
    weight += CS->weight;
    if (CS->iCol > nCols) nCols = CS->iCol;
  }

  nCols++;
  if (NI->Node == 0) printf ("ByColumn: Clusters = %d, Cols = %d, CPUs = %d\n", CellMgr->nClusters, nCols, NI->nNodes);

  if (nCols < NI->nNodes)
  {
    if (NI->Node == 0) 
    {
      printf ("Error: There are fewer columns (%d) than CPUs (%d)\n", nCols, NI->nNodes);
      printf ("ByColumn distribution requires at least as many columns as CPUs\n");
      printf ("Restart the job on %d or fewer nodes, or use a different distribution algorithm\n", nCols);
      fprintf (stderr, "Error: There are fewer columns (%d) than CPUs (%d)\n", nCols, NI->nNodes);
      fprintf (stderr, "Restart the job on %d or fewer nodes, or use a different distribution algorithm\n", nCols);
    }
    Abort (__FILE__, __LINE__, -10);
  }

  LocalIDs  = (int *)    calloc (NI->nNodes, sizeof (int));
  CPUweight = (double *) malloc (NI->nNodes * sizeof (double));
  for (i = 0; i < NI->nNodes; i++)
    CPUweight [i] = 0.0;
  ...
  {
    CS = CellMgr->Clusters [i];
    CS->Node = CS->iCol % NI->nNodes;
    CPUweight [CS->Node] = CS->weight;
  }
