...
//printf ("Report::Report: filename = %s\n", iRep->FileName);
  ...
  CellSequence = iRep->CellSequence;
  nThings   = 0;
  ThingList = NULL;
  RptBuf    = NULL;
  Out       = NULL;
  ...
  if (nThings > 0)
  {
    ...
    //Synapse Reports need to output an additional header to provide a map between synapses and
    //parent cells/cmps
    if( ReportOn == SYNAPSE_USE || ReportOn == SYNAPSE_RSE || ReportOn == SYNAPSE_UF ||
        ReportOn == SYNAPSE_SA || ReportOn == SYNAPSE_CA )
    {
        if( !(reportFlag & AVERAGE_SYN) )
        {
            int cellIndex = 0, thingIndex = 0;
            float* indexList = (float*) RptBuf;
            while( thingIndex < nThings )
            {
                for( int j=0; j<synapseCounts[cellIndex]; j++, thingIndex++ )
                    indexList[thingIndex] = cellIndex;
                cellIndex++;
            }
            Out->OpenFile( "a" );
            Out->StartOutput( -1, RptBuf );
            Out->FinishOutput( RptBuf );
            Out->CloseFile();
        }
    }
  }
...
void Report::GetCmpPtrs (T_REPORT *iRep, Random *RN)
{
  ...
  /* Description for new variables used in case of CELL_SEQUENCE = RANDOM:

     randIndex = Index of Random cell to report on.
     reportCellCount = Total number of cells to report on, if report prob. < 100%.
     indexIndicator = This array indicates which indices have been chosen,
     by putting in a value of 1. Eg. If cell id 23 was chosen, indexIndicator[23]
     would be 1.
     reportIndices = Holds cell indices which will be reported on. It is used
     to build "ThingList" variable below, in this function.
  */
  int i, j, nCells, *CellIDs, randIndex=0, reportCellCount=0, *indexIndicator=NULL, *reportIndices=NULL;
...
/* deleted from list according to random test.                               */
  ...
  if (CellIDs != NULL)
  {
    // If all cells are not going to be reported, then follow the CELL_SEQUENCE value specified in input file.
    // If absent, it defaults to ORIGINAL (linear sequence from first cell).
    if (iRep->Prob < 1.0)           /* not all cells will be reported */
    {
        ...
        {
            // Determine how the cells are reported. (ORIGINAL or RANDOM)
            if (CellSequence == ORIGINAL)    // select cells Sequentially from beginning
            {
                // Modification in the original algorithm to get exact number of cells.
                nCells = (int) round(nCells*iRep->Prob);
            }
            else if (CellSequence == RANDOM) // select cells randomly
            {
/* Start of New random reporting algorithm */
                // Calculate total number of cells to report on.
                reportCellCount = (int) round(nCells*iRep->Prob);

                // Allocate memory for array which will keep track of cell indices already
                // chosen for reporting. This helps avoid reporting duplicate cell indices.
                indexIndicator = (int *) calloc (nCells, sizeof (int));
                // Allocate memory for array which has the actual cell indices chosen for reporting.
                reportIndices = (int *) calloc (nCells, sizeof (int));

                for (i = j = 0; i < reportCellCount;)
                {                                    
                    prob = RN->Rand ();

                    // Calculate cell index to report. nCells is total number of cells to report from.
                    randIndex = (int) round(prob * nCells);

                    // Check if this cell index was chosen earlier. If yes (i.e. 1), skip it.
                    // If no (i.e. 0), add the cell index to the array of indices to report on.
                    if (indexIndicator[randIndex] == 0)
                    {
                        // Mark as selected
                        indexIndicator[randIndex]=1;
                        reportIndices[j++] = CellIDs[randIndex];
                        i++;
                    }
                }
                nCells = j;
                free (indexIndicator);
/* End of New random reporting algorithm - clean up ("free") is present at end of this function. */                
            }
        }
    }
...
/* the ThingList.                                                            */
    if (nCells > 0)
    {
      ...
      for (i = 0; i < nThings; i++)
      {
        // Start - Following statement has been changed to comply with new algorithm and requirements.
        if ((iRep->Prob < 1.0) && (CellSequence == RANDOM)) // specifically for prob. < 100% and RANDOM CELL_SEQUENCE
            Cel = TheBrain->Cells [reportIndices [i]];
        else							     // Default (original) code
    ...
    // Free reportIndices array, only if it was allocated memory.
    if ((iRep->Prob < 1.0) && (CellSequence == RANDOM))
        free (reportIndices);
  }  
}
...
void Report::GetSynPtrs (T_REPORT *iRep, Random *RN)
{
    ...
    //if( reportFlag & AVERAGE_SYN )
    //always allocate and populate the synapse counts array
    ...
    //If we are averaging synapses. then we will allocate less space for the report buffer
    if( reportFlag & AVERAGE_SYN )
      nThings = nCells;
